...
node {
    ...
    stage('npm install').withEnv('PATH=$PATH:${WORKSPACE}/node') {
    ...
    stage('build docker') {
        ...
        dockerImage = docker.build('xiemingzhi/jhipsterapp', 'target/docker')
    }

    stage('publish docker') {
        docker.withRegistry('https://registry.hub.docker.com', 'docker-xiemingzhi') {
